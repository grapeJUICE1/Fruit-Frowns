...
class Game:
    def __init__(self):
        ...
        self.__fruitTypes = ['apple' , 'orange' , 'banana' , 'pineapple' , 'coconut' , 'watermelon' , 'bomb']
    ...
    def createFruits(self):
        ...
        return [Fruit(name = fruit[i-1] , pos = (random.randint(300,GameConstants.SCREEN_DIMENSIONS[0]-200) , random.randint(GameConstants.SCREEN_DIMENSIONS[1] , GameConstants.SCREEN_DIMENSIONS[1]+20)) , angle=random.randint(70,115) ,sprite = pygame.transform.scale(pygame.image.load(os.path.join("Assets" , f"{fruit[i-1]}.png")) ,  GameConstants.FRUIT_SIZE) , color=f"{FruitColors[fruit[i-1]].value}") for i in range(random.randint(4,7))]
