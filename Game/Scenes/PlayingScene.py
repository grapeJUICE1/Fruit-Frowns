...
class PlayingGameScene(Scene):
    ...
    def render(self):
        ...
        for _ in range(self.game.getLives + 1):
        ...
        self.addText(
            f"{self.game.getScore}",
            GameConstants.SCREEN_DIMENSIONS[0] // 2 - 50,
            30,
            size=50,
        )
        ...
        if self.Player.draw:
            ...
            if self.Player.last_pos is not None:
                pygame.draw.line(
                    self.game.getScreen,
                    (255, 0, 0),
                    self.Player.last_pos,
                    self.mouse_position,
                    8,
                )
                GameConstants.SWORD_SOUNDS.play()

            # store player last pos before assigning it to mouse_position
            # will need this to check collison between fruit and player
            # as for that method , i will need this
            self.player_last_pos = self.Player.last_pos
    ...
    def renderBall(self, fruit):
        ...
        fruit.y = int(
            fruit.Y - (fruit.vy * fruit.ty - (fruit.gravity / 2) * fruit.ty * fruit.ty)
        )
        if fruit.x <= 0:
    ...
    def handleEvents(self, events):
        ...
        for event in events:
            ...
            for fruit in self.fruits:
                if (
                    fruit in self.fruits
                    and fruit.tossed == True
                    and (fruit.y > GameConstants.SCREEN_DIMENSIONS[1])
                ):
                    self.fruits.remove(fruit)
                    if fruit.name != "bomb":
                        self.game.reduceLives(1)

                if fruit in self.fruits and fruit.checkCollision(
                    self.Player, self.player_last_pos, self.mouse_position
                ):
                    ...
                    self.splashes.append(
                        [
                            pygame.image.load(
                                os.path.join(
                                    "Assets", f"splash_{fruit.color}_small.png"
                                )
                            ),
                            (fruit.x, fruit.y),
                            False,
                    ...
                    if fruit.name != "bomb":
                        self.cutFruits.append(
                            [
                                pygame.transform.scale(
                                    pygame.image.load(
                                        os.path.join(
                                            "Assets", f"{fruit.name}_half_1.png"
                                        )
                                    ),
                                    GameConstants.FRUIT_HALF_SIZE,
                                ),
                                [fruit.x, fruit.y],
                            ]
                        )
                        self.cutFruits.append(
                            [
                                pygame.transform.scale(
                                    pygame.image.load(
                                        os.path.join(
                                            "Assets", f"{fruit.name}_half_2.png"
                                        )
                                    ),
                                    GameConstants.FRUIT_HALF_SIZE,
                                ),
                        ...
                        pygame.mixer.Channel(self.soundChannel).play(
                            pygame.mixer.Sound(
                                os.path.join(
                                    "Assets",
                                    "Sound",
                                    f"Impact-{fruit.name.capitalize()}.wav",
                                )
                            )
                        )
                    else:
